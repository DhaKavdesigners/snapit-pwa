...
export const TopBar: React.FC = () => {
  ...
  // Short and sweet location formatting (not lengthy)
  const shortLocationTitle = userProfile?.landmark?.trim() 
    ? userProfile.landmark.trim() 
    : userLandmark?.trim() 
      ? userLandmark.trim() 
      : 'Robertsonpet';

  const shortLocationArea = userProfile?.addressLine2?.trim()
    ? `${userProfile.addressLine2.trim()}, KGF`
    : 'KGF, Karnataka';

  const currentDisplayLocation = shortLocationTitle;
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-gray-100 shadow-xs px-4 py-3 flex flex-col gap-2.5">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <Link to="/" className="flex items-center gap-2.5 shrink-0 group">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="bg-gradient-to-r from-amber-400 to-amber-500 text-amber-950 font-black text-[9.5px] px-2 py-0.5 rounded-full shadow-xs flex items-center gap-0.5 uppercase tracking-wider animate-pulse">
        ...
        {/* Row 2: Colorful, Dynamic & Short "Deliver to" Card */}
        <button
          onClick={() => setIsLocationModalOpen(true)}
          className="w-full bg-gradient-to-r from-emerald-50/90 via-white to-teal-50/70 hover:from-emerald-100/70 hover:to-teal-100/70 border border-emerald-200/90 hover:border-emerald-400 rounded-2xl p-2.5 px-3 flex items-center justify-between gap-3 transition-all active:scale-[0.99] text-left group shadow-xs"
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h4 className="text-sm font-black text-gray-900 truncate leading-tight tracking-tight mt-0.5">
                {shortLocationTitle}
          ...
          <div className="bg-emerald-600 group-hover:bg-emerald-700 text-white text-[11px] font-black px-2.5 py-1 rounded-full shadow-xs flex items-center gap-1 shrink-0 transition-colors">
            <span>Change</span>
